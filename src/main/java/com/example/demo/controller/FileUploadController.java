...
public class FileUploadController {
    ...
    @ApiOperation("文件上传接口")
    @PostMapping("/upload")
    public String up(@RequestBody MultipartFile headImg) throws IOException {
//        文件名称
...
//        文件上传的路径
        ...
        FileUtil.writeBytes(headImg.getBytes(),rootFilePath); //把文件写入路径
        System.out.println(headImg);
        return "http://" + ip + ":" + port + "/files/" + flag; //返回结果url
    }
    @ApiOperation("文件下载接口")
    @GetMapping("/{flag}")
